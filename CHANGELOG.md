...
### 0.21.105
 ...
 - **Fix a prod HTTP `/rpc` memory leak: interface connections leaked when entry failed.** A client polling the stateless `POST /rpc` path (which builds a workspace interface via `store.get_workspace_interface(...)`) drove the server's memory up ~625 MiB/day (`rpc_connections.created_total` climbing while `closed_total` stayed flat — 844 vs 436 on a bloated pod, with retained `RedisRPCConnection`≈`AsyncExitStack` 1:1 and ~50 method-proxy coroutines each). **Root cause:** `get_workspace_interface()` creates the `RedisRPCConnection` + RPC peer **synchronously in the factory**, *before* `__aenter__` runs; `__aenter__` then calls `get_manager_service()` — an RPC to the workspace manager bounded by `timeout`. Under load that call times out, so `__aenter__` raises — and **Python does not call `__aexit__` when `__aenter__` raised**, so the already-created connection (registry entry + event-bus handlers + RPC peer) was never disconnected. A second latent bug compounded it: the old `__aexit__` "force cleanup" fallback nulled `conn._filtered_handler`/`_handle_message` but never called `event_bus.off()`, so the `filtered_handler` closure stayed registered on the `{ws}/*:msg` pattern and kept pinning the connection even after it was popped from the registry. **Fix** (`hypha/core/store.py`, `WorkspaceInterfaceContextManager`): extract a single idempotent `_cleanup()` driven by `RedisRPCConnection.disconnect()` — the only method that both `event_bus.off()`s the handlers **and** increments `closed_total` — and call it from **both** `__aexit__` and `__aenter__`-on-failure. A timed-out entry now fully closes its connection. Tests (`tests/test_http_rpc_interface_leak.py`, in-process/fakeredis): forcing `__aenter__` to fail gives `created=40/closed=0/unclosed=40` **before** the fix and `created=40/closed=40/unclosed=0`, 0 objects retained **after**; happy-path and cancel-mid-request also green. (Note: the pre-existing `tests/test_http_anonymous_cleanup.py` measured `RedisRPCConnection._connections` in the *test* process while the server ran in a *subprocess*, so it never actually observed the server registry — which is why this leak slipped through; the new test drives the store in-process.)
