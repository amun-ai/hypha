...
### 0.21.119
 ...
 - **Unknown/inaccessible workspace on the HTTP apps path returns a clean 404 instead of a 500 + traceback (#0014, log hygiene).** A GET to `/{workspace}/apps/{service}/...` for a workspace that does not exist (and cannot be auto-created for the caller) raised a bare `KeyError` inside `get_workspace_interface`, which the ASGI handler caught with its **generic `except Exception`** → HTTP **500** and a full traceback logged at **ERROR** (`ERROR:http:Error in ASGI service: 'Workspace <ws> does not exist or is not accessible'`, ~1/24h in prod). An unknown workspace is a **client error (404)**, not a server fault, and must not spew a traceback into the ASGI error log (cf. V5 exception-info leakage). **Fix:** `store.load_or_create_workspace` now raises a dedicated **`WorkspaceNotFoundError`** — a **subclass of `KeyError`**, so the existing message contract (and any `except KeyError` handler, e.g. the other HTTP endpoints at http.py:833/1018/1203/1328) is preserved — and the ASGI apps handler catches it **specifically** (before the generic handler) → **404** + a single `logger.warning` (no traceback). Any *other* `KeyError` still surfaces as a 500 (non-masking — a genuine internal bug is not hidden behind a 404). The ASGI apps path was the only site with a generic `except Exception`; the sibling endpoints already narrow-catch `KeyError`. Flagged by the kth-k8s nightly platform review. `hypha/core/store.py::WorkspaceNotFoundError` + `hypha/http.py`; test `tests/test_asgi_unknown_workspace.py`.

### 0.21.118

 - **HTTP-streaming self-heals a WEDGED connection instead of dropping messages forever (#0012).** A prod incident: a client's retry-flood (frontend retrying not-found sessions in a tight no-backoff loop) filled a `svamp-machine`'s `/rpc` **downstream** queue; `send_to_queue` then logged `Message queue full … dropping message` at ~13/sec **continuously** and the machine went unresponsive (`getMachineInfo` timed out) — **offline to all users, with no auto-recovery** (it stayed wedged even after the flood stopped; only a *fresh* reconnect cleared it). Root cause: when a client stops reading its stream, the `StreamingResponse` `yield` blocks on TCP backpressure, so the stream loop never returns to `queue.get()` — the queue stays full and drops forever. Distinct from #0011 (that reaps *dead*/idle clients; a wedged client is *alive* and upstream-active, so the liveness sweep correctly doesn't touch it). **Fix:** count **consecutive** downstream-queue drops per connection (reset on any successful enqueue, so a slow-but-*draining* client never trips it), and once a connection is clearly not draining (`HYPHA_STREAM_MAX_QUEUE_DROPS`, default 200), the liveness sweep **force-recycles** it — cancelling the stuck stream task aborts the backpressured `yield`, the GET stream ends, and the client reconnects with a fresh empty queue (auto-recovery within ≤ one sweep). The client is alive, so its services are *not* reaped (it re-registers on reconnect). `hypha/http_rpc.py`; tests `tests/test_http_streaming_reaper.py`. (A companion client-side retry-bound + optional server fail-fast routing for *sustained* floods are follow-ups.)

### 0.21.117

 - **HTTP-streaming liveness reaper — half-open `/rpc` clients no longer ghost their service registrations forever (#0011).** A client on the HTTP-streaming transport (`/rpc`) that died **half-open** (e.g. a container hard-removed / `docker compose down` yanks the veth, so **no FIN** reaches the server) was never reaped: the stream loop only detects loss via `request.is_disconnected()`, which never fires without an `http.disconnect` event. So the client's service registrations (e.g. a `svamp-machine` `built-in` service) lingered in `list_services` indefinitely — a **ghost** that outlived the daemon for hours and only cleared on a server restart (an RPC to it, e.g. `getMachineInfo`, honestly timed out routing to the dead peer). WebSockets were unaffected — they already reap via `wait_for(receive(), idle_timeout)`; the gap was **streaming-only**. **Fix:** `HTTPStreamingRPCServer` now runs a background liveness sweep. A stream with no upstream POST for `HYPHA_STREAM_IDLE_THRESHOLD` (default 60s) becomes a *candidate* only — idle-but-live is normal for streaming, so idle **never** reaps by itself; the sweep then **actively RPC-pings** each candidate (`ping_client` → the client's built-in service) and reaps **only non-responders** via `store.remove_client` → `delete_client`, closing the ghost stream. A healthy idle machine answers the ping and is refreshed (never false-reaped); if liveness can't be verified it fails safe (no reap). Crucially, a client is reaped **only after N *consecutive* ping misses** (`HYPHA_STREAM_MAX_PING_MISSES`, default 3; reset on any answer), never on a single miss — the streaming ping is a full HTTP POST round-trip (slower than a WS frame), so a live-but-loaded machine can miss one and must not be false-reaped. Server-only, no client change, scoped to streaming. Config: `HYPHA_STREAM_IDLE_THRESHOLD` (60s), `HYPHA_STREAM_SWEEP_INTERVAL` (30s), `HYPHA_STREAM_PING_TIMEOUT` (5s), `HYPHA_STREAM_MAX_PING_MISSES` (3) — a half-open ghost is reaped in ~idle_threshold + N×sweep_interval (~150s default) instead of forever. `hypha/http_rpc.py`; tests `tests/test_http_streaming_reaper.py`.

### 0.21.116

 - **`--from-env` no longer silently clobbers explicit CLI args (#0006, item 1 — custom-auth DX).** `create_application`'s `--from-env` path did `setattr(args, key, value)` for **every** environment-derived arg, unconditionally overwriting explicitly-provided command-line flags — so a `--startup-functions=...` passed on the CLI was silently replaced by the environment default and a custom auth provider never loaded (Auth0 kept being served), costing a first-time integrator a full deploy+debug cycle. Fix: the merge now lets the **CLI win over env** — an environment value is applied only where the CLI did not explicitly set the arg (its value still equals the parser default); when the CLI did set it, the CLI value is kept and a **WARNING** names each env value being ignored. The server also now logs its **effective `startup_functions`** at boot, so an integrator can confirm at a glance that their module actually loaded. `hypha/server.py::merge_env_into_args` + `create_application`; tests `tests/test_from_env_cli_precedence.py`. (Remaining #0006 items — extra_handlers authenticated context, `check()` return contract docs, token-revocation hook, indexed auth-store lookup, and the full "build a custom auth provider" guide — are tracked as follow-ups.)

### 0.21.115

 - **Log hygiene: daemon apps installed for an UNAVAILABLE worker type no longer ERROR-spam on every restart (#0005).** Prod (scilifelab-2-dev) logged `ERROR:apps:Failed to start daemon app ... 'No server app worker found for type: compute-app'` ~11x on every restart, for demo apps (cap-test / canary-demo / ab-demo / rolling-demo / git-probe) whose type (`compute-app`) has no registered worker in that deployment. `compute-app` is not a hypha worker type at all — so this is an **environmental** condition (the worker is simply absent, e.g. an app installed for a custom/external worker that isn't present), **not** a broken app or a hypha bug. Fix: `hypha/apps.py` now raises a dedicated **`WorkerNotFoundError`** (subclass of `Exception`, so existing `except Exception` handlers are unaffected) wherever no worker supports an app type, and `prepare_workspace`'s daemon-autostart loop catches it **before** the generic handler → a single clear **WARNING** ("Skipping daemon app … worker type unavailable"), skipping the app. Genuine start failures still log at **ERROR** with detail. Same log-hygiene family as 0.21.107 / 0.21.113. `hypha/apps.py`; tests `tests/test_daemon_worker_unavailable.py`.

### 0.21.114

 - **`recipient_can_delete` — recipient self-delete of own children in private-children collections (#0010).** Companion to #0009. A collection configured with `{"recipient_can_delete": true, "recipient_field": "<field>"}` lets a recipient delete (ack / prune) **only** the children addressed to them. Deleting a child of a collection normally requires **two** grants that the drop-box `@:lf+` (and a per-child `rd+` ACL) deliberately withhold: `delete` on the child (only the `*`/full alias, or workspace-admin) **and** `detach` on the parent collection (`rw`/`rw+`/`*`). This adds a narrow server-side fast-path in `delete()`: the artifact is fetched first (no permission gate), and if the parent has the flag and `child.manifest[recipient_field] == caller.email`, the delete is authorized **bypassing both gates — but only for the caller's own child**; otherwise it falls through to the normal strict checks (unchanged). Recipient-only (no sender-retract — a daemon un-share runs as admin and is already covered by the admin bypass). Single-child only (`version=None`, non-recursive); recursive/collection deletes stay admin-only. Composes with #0009 so a recipient can only delete what they can see (their own). Powers `svamp notify` message ack / self-service TTL prune without a standing admin cron. `hypha/artifact.py::delete`; test `tests/test_artifact.py::test_recipient_can_delete_own_child`.

### 0.21.113

 - **Log hygiene: routine JWT expiry no longer emits a full ERROR traceback (#0008).** A client presenting an expired or invalid token on connect is an **expected** client condition (the client should refetch), not a server fault — yet it was logging a multi-frame `exc_info=True` ERROR (~30/24h of pure noise, flagged by the nightly review). Path: bad client token → `core/auth.py::valid_token` raises `HTTPException(401)` → `websocket.py::authenticate_user` re-raised it as a generic `RuntimeError` → the connection-setup `except Exception` catch-all logged the full traceback at ERROR. **Fix:** `authenticate_user` now raises a dedicated `AuthenticationError` (from the `HTTPException`), caught in the setup block **before** the generic handler → a single WARNING line (no traceback) + close **1008**. Genuine setup errors still raise through to the ERROR+traceback path (unchanged). Same log-hygiene family as 0.21.106/0.21.107. `hypha/websocket.py`; regression test `tests/test_websocket_lifecycle_hardening.py::test_expired_or_invalid_token_logs_warning_not_error_traceback` (drives the ASGI endpoint with an invalid token, asserts no ERROR/exc_info, exactly one WARNING, 1008 close).
...
### 0.21.111
 ...
 - **Fix two low-volume WebSocket teardown races** (both caught/non-fatal, ~2/24h each in prod; flagged by the nightly review). The 0.21.107 send-after-close fix is confirmed holding (0/24h).
   - **`register_client()` crashed on `self._event_bus` after disconnect** (`hypha/core/__init__.py`). On a reconnect via reconnection-token immediately followed by a 1001 disconnect, the background `register_client()` task (which `RedisRPCConnection.on_message` schedules to `subscribe_to_client_events`) could run just as `disconnect()` set `self._event_bus = None`, raising `AttributeError: 'NoneType' object has no attribute 'subscribe_to_client_events'`. **Fix:** `register_client()` captures the bus into a local and returns if it is already `None` — the local capture also closes the window where a concurrent `disconnect()` nils `self._event_bus` mid-await. Same teardown-race class as the 0.21.106 `filtered_handler` `None` guard.
   - **`WebSocketDisconnect` during the auth handshake propagated as "Exception in ASGI application"** (`hypha/websocket.py`). If a client opens the socket and goes away (code 1005, no close frame) before sending its auth payload, `websocket.receive_text()` raised `WebSocketDisconnect`, which the existing `except asyncio.TimeoutError` didn't catch → uvicorn logged it at ERROR. **Fix:** catch `WebSocketDisconnect` on the auth read and return cleanly (a client bailing mid-handshake is benign; logged at DEBUG).
   - Tests: `tests/test_websocket_lifecycle_hardening.py` (`test_register_client_task_after_teardown_no_error`; `test_ws_disconnect_during_auth_handshake_no_error` drives the ASGI endpoint with a pre-auth disconnect and asserts it doesn't raise).
 - **Note — feature rollup:** deploying 0.21.111 also ships the **0.21.108–0.21.110** changes (local-auth email verification via Resend; embedded no-popup iframe login for local + custom auth). These are backward-compatible and CI-green, but are a **feature rollout**, not pure hardening — operators should treat the deploy as a conscious feature call, not a silent ride-along.
