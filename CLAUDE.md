...
**Deleting a collection child needs TWO grants, not one (learned via #0010).** `delete()` (`hypha/artifact.py`) requires `delete` on the *child* — which lives ONLY in the `*`/full alias (there is no granular delete perm; `rw`/`rw+`/`rd+` do NOT include it), or workspace-admin — AND, separately, `detach` on the *parent* collection (`rw`/`rw+`/`*`, NOT in `lf+`/`rd+`). So a drop-box collection granting `@:lf+` (list + create-child) gives recipients no way to delete their own messages, and a per-child ACL can't fix it (only `*` grants delete, and the parent-`detach` gate is independent). The **`recipient_can_delete`** flag (`{"recipient_can_delete": true, "recipient_field": "<field>"}`, companion to `private_children`) adds a narrow fast-path in `delete()`: fetch the child first (no gate), and if the parent has the flag and `child.manifest[<recipient_field>] == caller.email`, authorize bypassing BOTH gates — own-children only, single-child only (`version=None`, non-recursive; recursive/collection delete stays admin-only). Powers recipient ack / self-prune (`svamp notify`) without granting `@` broad delete/detach or `*` to recipients. `hypha/artifact.py::delete`; test `test_recipient_can_delete_own_child`.
...
| A new per-client connection SUPERSEDES an old one without cancelling the old coroutine | The old coroutine parked in `await websocket.receive()` never exits, so its `finally` cleanup never runs → it leaks the connection forever. Prod WS OOM (0.21.105): reconnect bumped the generation + overwrote `_websockets[conn_key]` but left the old coroutine parked; clients accumulated 9–14 zombie generations (`live RedisRPCConnection ≫ registry active`) | Track the task per `conn_key` and CANCEL the previous generation's task on supersede so its `finally` runs cleanup immediately. Don't rely on an idle timeout to reap it |
| Idle timeout keyed on "time since last inbound frame" treated as a liveness check | Any inbound frame resets it — including client keepalive `ping` messages — so an abandoned/half-open-but-noisy peer parks forever and is never reaped. It measures *silence*, not *liveness* | For true liveness use server-initiated ping + pong DEADLINE (or the ASGI server's `ws_ping_interval`/`ws_ping_timeout`); reserve the inbound-frame idle timeout for detecting genuinely silent peers |
| Asserting process **RSS** as a proxy for a streaming path's **live/O(1) memory** on glibc | glibc arenas (`MALLOC_ARENA_MAX=2`) RETAIN freed per-object buffers ~proportional to bytes CHURNED; `malloc_trim` cannot return a partially-live arena, so RSS scales ~linearly with THROUGHPUT even when live memory is bounded. A single git clone's RSS was ~2.0× pack size on Linux (documented PR#977 "1-2×") while live pack-data was one 4MiB chunk. macOS's eager-return allocator hides this, so an RSS-slope test PASSES on macOS and FAILS on Linux — it measures allocator retention, not the code | Measure the **bounded live buffer at its source**, platform-independently (e.g. drive `_S3RangeReader` over real MinIO and assert `reader._buf` ≤ one chunk regardless of object size — `tests/test_git_memory_streaming.py::test_range_reader_resident_bounded_by_chunk`). Note the server test fixture is a **subprocess**, so in-process monkeypatching of server code won't work — unit-test the mechanism in-process instead |
...
- **Fix:** probe with `setup = getattr(svc, "setup", None)` and gate on `if callable(setup):`. "No setup method" becomes a silent no-op; only a `setup()` that actually **exists and raises** is an ERROR — now formatted `f"{type(e).__name__}: {e!r}"` so the detail names the real failure (e.g. `RemoteException: RemoteException('RemoteError:boom-in-setup…')`), never the bare attribute name. The resolve step (`get_remote_service`) gets its own narrower `except` so a genuine resolution failure is still distinguishable from a benign missing-`setup`.
- **Key Lesson:** never truth-test an **optional** attribute on a Munch/ObjectProxy with `if proxy.attr:` — that raises on absence and, wrapped in a broad `except`, mints a benign ERROR with a meaningless detail. Use `getattr(proxy, "attr", None)`. And when you DO log a caught exception, format `type(e).__name__: {e!r}` — `{e}` alone on an `AttributeError` is just the attribute name. Note `hypha/workers/__init__.py:38` has the same `if svc.setup:` shape but is intentionally left (worker services define `setup`; out of scope). The other two #0044 findings are pure level demotions: `artifact.py::search` query-planner narration (`search: stage parameter value`, `Adding stage filter condition`, `Adding condition to return ONLY …`) INFO→DEBUG (~300k lines/24h, a hot listing path), and `http_rpc.py::_handle_rpc_post` catching `starlette.requests.ClientDisconnect` **specifically** (client hung up before/while the body arrived — a normal network event) → DEBUG + `499`, before the broad `except` that logged it as ERROR+traceback+500 (618/24h). Same log-hygiene family as #0008/#0019/#0020/#0021. Tests: `tests/test_log_hygiene_0044.py` (5, Docker-free, reproduce-before-fix: the 3 regression guards fail on old code, showing the exact prod `ERROR workspace:… Failed to run setup …: setup` line). 0.21.135.
