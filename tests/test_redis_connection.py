...
from hypha.core.store import RedisRPCConnection, RedisEventBus, UserInfo  # Update with the actual module path
...
async def user_info_admin():
    return UserInfo(id="admin1", is_anonymous=False, email="admin1@example.com", roles=["admin"])

@pytest_asyncio.fixture
async def user_info_user():
    return UserInfo(id="user1", is_anonymous=False, email="user1@example.com", roles=["user"])
...
async def test_dynamic_workspace_interaction(first_rpc, dynamic_workspace_rpc):
    ...
    dynamic_to_first_message = msgpack.packb({"to": "workspace1/client1", "content": "Hello from dynamic"})
    await dynamic_workspace_rpc.emit_message(dynamic_to_first_message)

    # Messages from fixed workspace client to dynamic client
    first_to_dynamic_message = msgpack.packb({"to": "*/dynamic_client", "content": "Reply from workspace1"})
...
async def test_message_emission_within_same_workspace(first_rpc, second_client_same_workspace):
    first_messages = []
    second_messages = []

    first_rpc.on_message(lambda data: first_messages.append(msgpack.unpackb(data)))
    second_client_same_workspace.on_message(lambda data: second_messages.append(msgpack.unpackb(data)))
...
async def test_message_emission_across_workspaces(first_rpc, another_rpc_connection):
    ...
    first_rpc.on_message(lambda data: first_messages.append(msgpack.unpackb(data)))
    another_rpc_connection.on_message(lambda data: another_messages.append(msgpack.unpackb(data)))

    await asyncio.sleep(0.1)

    message_to_another = msgpack.packb({"to": "workspace2/client2", "content": "Hello to workspace2"})
    message_to_first = msgpack.packb({"to": "workspace1/client1", "content": "Reply to workspace1"})
...
async def test_broadcast_message_within_workspace(first_rpc, second_client_same_workspace):
    first_messages = []
    second_messages = []
...
async def test_broadcast_message_does_not_cross_workspaces(first_rpc, another_rpc_connection, second_client_same_workspace):
    first_messages = []
    second_messages = []
    another_messages = []
