...
class TestCLI:
    ...
    def test_generate_token_all_options(self):
        ...
        args = parser.parse_args([
            ...
            "--expires-in", "1800",
            "--client-id", "full-test-client",
            "--user-id", "full-test-user",
            "--role", "admin",
            "--role", "developer",
...
class TestCLIIntegration:
    ...
    def running_server(self):
        ...
        opt = arg_parser.parse_args([
            "--host", "127.0.0.1",
            "--port", "9528",  # Use different port for testing
            "--enable-server-apps"
        ])
    ...
    def test_generate_token_cli_command(self):
        ...
        result = subprocess.run([
            sys.executable, "-m", "hypha.__main__", 
            "generate-token", 
            "--workspace", "cli-test-workspace",
            "--expires-in", "3600"
...
'''
    ...
    def test_server_subcommand_compatibility(self):
        ...
        assert result.returncode == 0
        assert ("--host" in result.stdout or 
