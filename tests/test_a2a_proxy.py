...
async def test_a2a_round_trip_service_consistency(fastapi_server, test_user_token):
    ...
    async def a2a_run(message, context=None):
        return await _handle_a2a_message(message, [echo_skill, greeting_skill, calculate_skill])
    ...
    session_info = await controller.start(a2a_app_info["id"], wait_for_service=agent_name, timeout=30)
    ...
    assert "Nice to meet you" in original_greeting and "Nice to meet you" in a2a_greeting
...
async def test_a2a_double_round_trip_reversibility(fastapi_server, test_user_token):
    ...
    first_session_info = await controller.start(first_a2a_app_info["id"], wait_for_service="first-agent", timeout=30)
    ...
    second_session_info = await controller.start(second_a2a_app_info["id"], wait_for_service="second-agent", timeout=30)
...
async def test_a2a_error_handling_and_debugging(fastapi_server, test_user_token):
    ...
    try:
        session_info = await controller.start(a2a_app_info["id"], wait_for_service="invalid-agent", timeout=10)
