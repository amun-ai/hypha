...
class TestServiceInfoOldFormatMigration:
    ...
    def test_extra_fields_preserved(self):
        """Extra fields like 'score' from vector search are preserved."""
        # Simulate search result dict (non-bytes, has extra 'score' field)
        data = {
            "id": "test-ws/client:svc",
            "name": "Test",
            "type": "generic",
            "config": json.dumps({"visibility": "public"}),
            "score": "0.95",
        }
        info = ServiceInfo.from_redis_dict(data, in_bytes=False)
        assert info.id == "test-ws/client:svc"
        ...
        assert "score" in dump
        assert dump["score"] == 0.95

    def test_extra_fields_preserved_bytes(self):
        """Extra fields preserved from bytes-encoded Redis data."""
        data = {
            b"id": b"test-ws/client:svc",
            b"name": b"Test",
            b"type": b"generic",
            b"config": json.dumps({"visibility": "public"}).encode(),
            b"score": b"0.95",
        }
        info = ServiceInfo.from_redis_dict(data, in_bytes=True)
        assert info.score == 0.95
