...
async def test_asgi(fastapi_server, test_user_token):
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.put(f"{SERVER_URL}/{workspace}/apps/test-asgi/api", json={"name": "Alice"})
        assert response.json()["message"] == "Hello Alice"
    
        response = await client.post(f"{SERVER_URL}/{workspace}/apps/test-asgi/api", json={"name": "Alice"})
        assert response.json()["message"] == "Hello Alice"
    
        response = await client.delete(f"{SERVER_URL}/{workspace}/apps/test-asgi/api")
        assert response.json()["message"] == "Deleted"
    
        response = await client.patch(f"{SERVER_URL}/{workspace}/apps/test-asgi/api", json={"name": "Alice"})
