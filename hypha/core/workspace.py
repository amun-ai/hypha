...
class WorkspaceManager:
    ...
    async def start_periodic_cleanup(self, interval: int = 300):
        """Start a background task to periodically clean up hanging clients.
        ...
        async def cleanup_task():
            while True:
                try:
                    ...
                    logger.info("Starting periodic hanging client cleanup")
                    
                    # Get all workspaces
                    workspace_keys = await self._redis.hkeys("workspaces")
                    cleaned_count = 0
                    
                    for workspace_data in workspace_keys:
                        workspace = workspace_data.decode()
                        ...
                        try:
                            ...
                            for client_key in client_keys:
                                ...
                                try:
                                    # Quick ping test
                                    ping_result = await self.ping_client(client_key, timeout=3)
                                    if ping_result != "pong":
                                        logger.warning(f"Periodic cleanup: removing unresponsive client {client_key}")
                                        context = {"user": self._root_user.model_dump(), "ws": workspace}
                                        await self.delete_client(
                                            client_id, workspace, self._root_user, 
                                            unload=False, context=context
                                        )
                                        cleaned_count += 1
                                except Exception as ping_error:
                                    logger.warning(f"Periodic cleanup: client {client_key} failed ping test: {ping_error}")
                                    try:
                                        context = {"user": self._root_user.model_dump(), "ws": workspace}
                                        await self.delete_client(
                                            client_id, workspace, self._root_user, 
                                        ...
                                        logger.error(f"Failed to cleanup hanging client {client_key}: {cleanup_error}")
                                        
                        except Exception as workspace_error:
                            logger.error(f"Error during periodic cleanup of workspace {workspace}: {workspace_error}")
                    
                    if cleaned_count > 0:
                        logger.info(f"Periodic cleanup completed: removed {cleaned_count} hanging clients")
                    else:
                        logger.debug("Periodic cleanup completed: no hanging clients found")
                        
                except Exception as e:
                    logger.error(f"Error in periodic hanging client cleanup: {e}")
        
        # Start the cleanup task in the background
        cleanup_task_handle = asyncio.create_task(cleanup_task())
        logger.info(f"Started periodic hanging client cleanup (interval: {interval}s)")
        return cleanup_task_handle
