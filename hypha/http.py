...
async def extracted_kwargs(
    ...
    if request.method == "GET":
        ...
        if not body:
            # Empty POST body is treated as no arguments (e.g., calling a method with no parameters)
            kwargs = {}
...
class HTTPRateLimitMiddleware:
    ...
    Exempts health/liveness/readiness endpoints and WebSocket upgrades to avoid
    interfering with Kubernetes probes and WS connections (which have their own
    ...
    async def __call__(self, scope, receive, send):
        ...
        # WebSocket upgrades have their own per-client rate limiting
    ...
    def __init__(
        ...
        async def health_diagnostics(req: Request) -> JSONResponse:
            ...
            try:
                ...
                event_bus_health = {
                    "circuit_breaker_open": event_bus._circuit_breaker_open,
                    "consecutive_failures": event_bus._consecutive_failures,
                    "max_failures": event_bus._max_failures,
                    "last_successful_connection": (
                        datetime.fromtimestamp(event_bus._last_successful_connection).isoformat()
                        if event_bus._last_successful_connection else None
                    ),
                }
