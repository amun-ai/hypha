...
# Regex to match any SQL that starts with SELECT (case-insensitive), followed by optional spaces
SELECT_QUERY_REGEX = re.compile(r'^\s*SELECT\b', re.IGNORECASE)
...
class ArtifactController:
    ...
    def __init__(
        ...
        if database_uri is None:
            database_uri = "sqlite+aiosqlite:///./artifacts.db"  # Default to SQLite
    ...
    async def create(
        ...
        if not user_info.check_permission(ws, UserPermission.read_write):
            raise PermissionError("User does not have write permission to the workspace.")
        ...
        try:
            ...
            async with session.begin():
                new_artifact = ArtifactModel(
                    ...
                    is_collection=(manifest["type"] == "collection"),
                )
    ...
    async def read(self, prefix, stage=False, context: dict = None):
        ...
        if not user_info.check_permission(ws, UserPermission.read):
            raise PermissionError("User does not have read permission to the workspace.")
        ...
        try:
            async with session.begin():
                ...
                if not manifest:
                    raise KeyError(f"No manifest found for artifact under prefix '{prefix}'.")
                ...
                if manifest.get("type") == "collection":
                    ...
                    summary_fields = manifest.get("summary_fields", ["id", "name", "description"])
                    collection = []
        ...
        try:
            async with session.begin():
                ...
                if not artifact or not artifact.stage_manifest:
                    raise KeyError(f"No staged manifest to commit for artifact '{prefix}'.")
                ...
                if parent_prefix:
                    ...
                    if parent_artifact and "collection_schema" in parent_artifact.manifest:
                        collection_schema = parent_artifact.manifest["collection_schema"]
                        try:
    ...
    async def search(self, prefix, keywords=None, filters=None, mode="AND", context: dict = None):
        """
        ...
        try:
            async with session.begin():
                ...
                backend = self.engine.dialect.name  # This will return 'postgresql', 'sqlite', etc.
                ...
                if filters:
                    for key, value in filters.items():
                        if "*" in value:  # Fuzzy matching
                            if backend == "postgresql":
                                condition = text(f"manifest->>'{key}' ILIKE '{value.replace('*', '%')}'")
                            else:
                                condition = text(f"json_extract(manifest, '$.{key}') LIKE '{value.replace('*', '%')}'")
                        else:  # Exact matching
                            if backend == "postgresql":
                                condition = text(f"manifest->>'{key}' = '{value}'")
                            else:
                                condition = text(f"json_extract(manifest, '$.{key}') = '{value}'")
                        conditions.append(condition)
            ...
            raise ValueError(f"An error occurred while executing the search query: {str(e)}")
        finally:
            await session.close()
    ...
    async def put_file(self, prefix, file_path, context: dict = None):
        ...
        try:
            async with session.begin():
                ...
                if not any(f["path"] == file_path for f in artifact.stage_manifest["files"]):
    ...
    async def remove_file(self, prefix, file_path, context: dict):
        ...
        try:
            async with session.begin():
                artifact = await self._get_artifact(session, ws, prefix)
                if not artifact or not artifact.stage_manifest:
                    raise KeyError(f"Artifact under prefix '{prefix}' is not in staging mode.")

                manifest = artifact.stage_manifest
                manifest["files"] = [f for f in manifest.get("files", []) if f["path"] != file_path]
                flag_modified(artifact, "stage_manifest")
                session.add(artifact)
                await session.commit()
        finally:
