...
from hypha.litellm.constants import (
    BASE_MCP_ROUTE,
...
from hypha.litellm.litellm_core_utils.safe_json_dumps import safe_dumps
from hypha.litellm.types.utils import (
    ModelResponse,
    ModelResponseStream,
...
from hypha.litellm import Router
from hypha.litellm._logging import verbose_proxy_logger, verbose_router_logger
from hypha.litellm.caching.caching import DualCache, RedisCache
from hypha.litellm.caching.redis_cluster_cache import RedisClusterCache
from hypha.litellm.constants import (
    DAYS_IN_A_MONTH,
...
from hypha.litellm.exceptions import RejectedRequestError
from hypha.litellm.integrations.SlackAlerting.slack_alerting import SlackAlerting
from hypha.litellm.litellm_core_utils.core_helpers import (
    _get_parent_otel_span_from_kwargs,
    get_litellm_metadata_from_kwargs,
)
from hypha.litellm.litellm_core_utils.credential_accessor import CredentialAccessor
from hypha.litellm.litellm_core_utils.litellm_logging import Logging as LiteLLMLoggingObj
from hypha.litellm.litellm_core_utils.sensitive_data_masker import SensitiveDataMasker
from hypha.litellm.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
from hypha.litellm.proxy._experimental.mcp_server.rest_endpoints import (
    router as mcp_rest_endpoints_router,
)
from hypha.litellm.proxy._experimental.mcp_server.server import app as mcp_app
from hypha.litellm.proxy._experimental.mcp_server.tool_registry import (
    global_mcp_tool_registry,
)
from hypha.litellm.proxy._types import *
from hypha.litellm.proxy.analytics_endpoints.analytics_endpoints import (
    router as analytics_router,
)
from hypha.litellm.proxy.anthropic_endpoints.endpoints import router as anthropic_router
from hypha.litellm.proxy.auth.auth_checks import (
    ExperimentalUIJWTToken,
    get_team_object,
    log_db_metrics,
)
from hypha.litellm.proxy.auth.auth_utils import check_response_size_is_safe
from hypha.litellm.proxy.auth.handle_jwt import JWTHandler
from hypha.litellm.proxy.auth.litellm_license import LicenseCheck
from hypha.litellm.proxy.auth.model_checks import (
    get_all_fallbacks,
...
from hypha.litellm.proxy.auth.user_api_key_auth import (
    user_api_key_auth,
    user_api_key_auth_websocket,
)
from hypha.litellm.proxy.batches_endpoints.endpoints import router as batches_router

## Import All Misc routes here ##
from hypha.litellm.proxy.caching_routes import router as caching_router
from hypha.litellm.proxy.common_request_processing import (
    ProxyBaseLLMRequestProcessing,
    create_streaming_response,
)
from hypha.litellm.proxy.common_utils.callback_utils import initialize_callbacks_on_proxy
from hypha.litellm.proxy.common_utils.debug_utils import init_verbose_loggers
from hypha.litellm.proxy.common_utils.debug_utils import router as debugging_endpoints_router
from hypha.litellm.proxy.common_utils.encrypt_decrypt_utils import (
    decrypt_value_helper,
    encrypt_value_helper,
)
from hypha.litellm.proxy.common_utils.html_forms.ui_login import html_form
from hypha.litellm.proxy.common_utils.http_parsing_utils import (
    _read_request_body,
    check_file_size_under_limit,
    get_form_data,
)
from hypha.litellm.proxy.common_utils.load_config_utils import (
    get_config_file_contents_from_gcs,
    get_file_contents_from_s3,
)
from hypha.litellm.proxy.common_utils.openai_endpoint_utils import (
    remove_sensitive_info_from_deployment,
)
from hypha.litellm.proxy.common_utils.proxy_state import ProxyState
from hypha.litellm.proxy.common_utils.reset_budget_job import ResetBudgetJob
from hypha.litellm.proxy.common_utils.swagger_utils import ERROR_RESPONSES
from hypha.litellm.proxy.credential_endpoints.endpoints import router as credential_router
from hypha.litellm.proxy.db.db_transaction_queue.spend_log_cleanup import SpendLogCleanup
from hypha.litellm.proxy.db.exception_handler import PrismaDBExceptionHandler
from hypha.litellm.proxy.discovery_endpoints import ui_discovery_endpoints_router
from hypha.litellm.proxy.fine_tuning_endpoints.endpoints import router as fine_tuning_router
from hypha.litellm.proxy.fine_tuning_endpoints.endpoints import set_fine_tuning_config
from hypha.litellm.proxy.google_endpoints.endpoints import router as google_router
from hypha.litellm.proxy.guardrails.guardrail_endpoints import router as guardrails_router
from hypha.litellm.proxy.guardrails.init_guardrails import (
    init_guardrails_v2,
    initialize_guardrails,
)
from hypha.litellm.proxy.health_check import perform_health_check
from hypha.litellm.proxy.health_endpoints._health_endpoints import router as health_router
from hypha.litellm.proxy.hooks.model_max_budget_limiter import (
    _PROXY_VirtualKeyModelMaxBudgetLimiter,
)
from hypha.litellm.proxy.hooks.prompt_injection_detection import (
    _OPTIONAL_PromptInjectionDetection,
)
from hypha.litellm.proxy.hooks.proxy_track_cost_callback import _ProxyDBLogger
from hypha.litellm.proxy.image_endpoints.endpoints import router as image_router
from hypha.litellm.proxy.litellm_pre_call_utils import add_litellm_data_to_request
from hypha.litellm.proxy.management_endpoints.budget_management_endpoints import (
    router as budget_management_router,
)
from hypha.litellm.proxy.management_endpoints.callback_management_endpoints import (
    router as callback_management_endpoints_router,
)
from hypha.litellm.proxy.management_endpoints.common_utils import _user_has_admin_view
from hypha.litellm.proxy.management_endpoints.customer_endpoints import (
    router as customer_router,
)
from hypha.litellm.proxy.management_endpoints.internal_user_endpoints import (
    router as internal_user_router,
)
from hypha.litellm.proxy.management_endpoints.internal_user_endpoints import user_update
from hypha.litellm.proxy.management_endpoints.key_management_endpoints import (
...
from hypha.litellm.proxy.management_endpoints.mcp_management_endpoints import (
    router as mcp_management_router,
)
from hypha.litellm.proxy.management_endpoints.model_management_endpoints import (
    _add_model_to_db,
...
from hypha.litellm.proxy.management_endpoints.organization_endpoints import (
    router as organization_router,
)
from hypha.litellm.proxy.management_endpoints.scim.scim_v2 import scim_router
from hypha.litellm.proxy.management_endpoints.tag_management_endpoints import (
    router as tag_management_router,
)
from hypha.litellm.proxy.management_endpoints.team_callback_endpoints import (
    router as team_callback_router,
)
from hypha.litellm.proxy.management_endpoints.team_endpoints import router as team_router
from hypha.litellm.proxy.management_endpoints.team_endpoints import (
    update_team,
    validate_membership,
)
from hypha.litellm.proxy.management_endpoints.ui_sso import (
    get_disabled_non_admin_personal_key_creation,
)
from hypha.litellm.proxy.management_endpoints.ui_sso import router as ui_sso_router
from hypha.litellm.proxy.management_endpoints.user_agent_analytics_endpoints import (
    router as user_agent_analytics_router,
)
from hypha.litellm.proxy.management_helpers.audit_logs import create_audit_log_for_update
from hypha.litellm.proxy.middleware.prometheus_auth_middleware import PrometheusAuthMiddleware
from hypha.litellm.proxy.openai_files_endpoints.files_endpoints import (
    router as openai_files_router,
)
from hypha.litellm.proxy.openai_files_endpoints.files_endpoints import set_files_config
from hypha.litellm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
    passthrough_endpoint_router,
)
from hypha.litellm.proxy.pass_through_endpoints.llm_passthrough_endpoints import (
    router as llm_passthrough_router,
)
from hypha.litellm.proxy.pass_through_endpoints.pass_through_endpoints import (
    initialize_pass_through_endpoints,
)
from hypha.litellm.proxy.pass_through_endpoints.pass_through_endpoints import (
    router as pass_through_router,
)
from hypha.litellm.proxy.prompts.prompt_endpoints import router as prompts_router
from hypha.litellm.proxy.public_endpoints import router as public_endpoints_router
from hypha.litellm.proxy.rerank_endpoints.endpoints import router as rerank_router
from hypha.litellm.proxy.response_api_endpoints.endpoints import router as response_router
from hypha.litellm.proxy.route_llm_request import route_request
from hypha.litellm.proxy.spend_tracking.cloudzero_endpoints import router as cloudzero_router
from hypha.litellm.proxy.spend_tracking.spend_management_endpoints import (
    router as spend_management_router,
)
from hypha.litellm.proxy.spend_tracking.spend_tracking_utils import get_logging_payload
from hypha.litellm.proxy.types_utils.utils import get_instance_fn
from hypha.litellm.proxy.ui_crud_endpoints.proxy_setting_endpoints import (
    router as ui_crud_endpoints_router,
)
from hypha.litellm.proxy.utils import (
    PrismaClient,
    ProxyLogging,
    ProxyUpdateSpend,
    _cache_user_row,
...
from hypha.litellm.proxy.vector_store_endpoints.endpoints import router as vector_store_router
from hypha.litellm.proxy.vertex_ai_endpoints.langfuse_endpoints import (
    router as langfuse_router,
)
from hypha.litellm.router import (
    AssistantsTypedDict,
    Deployment,
    LiteLLM_Params,
    ModelGroupInfo,
)
from hypha.litellm.scheduler import FlowItem, Scheduler
from hypha.litellm.secret_managers.aws_secret_manager import load_aws_kms
from hypha.litellm.secret_managers.google_kms import load_google_kms
from hypha.litellm.secret_managers.main import (
    get_secret,
    get_secret_bool,
    get_secret_str,
    str_to_bool,
)
from hypha.litellm.types.integrations.slack_alerting import SlackAlertingArgs
from hypha.litellm.types.llms.anthropic import (
    AnthropicMessagesRequest,
    AnthropicResponse,
    AnthropicResponseContentBlockText,
    AnthropicResponseUsageBlock,
)
from hypha.litellm.types.llms.openai import HttpxBinaryResponseContent
from hypha.litellm.types.proxy.management_endpoints.model_management_endpoints import (
    ModelGroupInfoProxy,
)
from hypha.litellm.types.proxy.management_endpoints.ui_sso import (
    DefaultTeamSSOParams,
    LiteLLM_UpperboundKeyGenerateParams,
)
from hypha.litellm.types.realtime import RealtimeQueryParams
from hypha.litellm.types.router import DeploymentTypedDict
from hypha.litellm.types.router import ModelInfo as RouterModelInfo
from hypha.litellm.types.router import RouterGeneralSettings, updateDeployment
from hypha.litellm.types.scheduler import DefaultPriorities
from hypha.litellm.types.secret_managers.main import (
    KeyManagementSettings,
    KeyManagementSystem,
)
from hypha.litellm.types.utils import CredentialItem, CustomHuggingfaceTokenizer
from hypha.litellm.types.utils import ModelInfo as ModelMapInfo
from hypha.litellm.types.utils import RawRequestTypedDict, StandardLoggingPayload
from hypha.litellm.utils import _add_custom_logger_callback_to_specific_event

try:
    from hypha.litellm._version import version
except Exception:
    version = "0.0.0"
...
async def proxy_shutdown_event():
    ...
    if "langfuse" in litellm.success_callback:
        try:
            ...
            from hypha.litellm.utils import langFuseLogger
...
def get_openapi_schema():
    ...
    from hypha.litellm.proxy.common_utils.custom_openapi_spec import CustomOpenAPISpec
...
class ProxyConfig:
    ...
    def _init_cache(
        ...
        from hypha.litellm import Cache
    ...
    async def load_config(  # noqa: PLR0915
        ...
        if litellm_settings:
            ...
            for key, value in litellm_settings.items():
                if key == "cache" and value is True:
                    ...
                    from hypha.litellm.caching.caching import Cache
                    ...
                    from hypha.litellm.integrations.dotprompt import (
                    ...
                    from hypha.litellm.types.router import ModelGroupSettings
                    ...
                    from hypha.litellm.utils import custom_llm_setup
        ...
        if prompts:
            from hypha.litellm.proxy.prompts.init_prompts import init_prompts
    ...
    def _init_non_llm_configs(self, config: dict):
        ...
        if mcp_servers_config:
            from hypha.litellm.proxy._experimental.mcp_server.mcp_server_manager import (
        ...
        if vector_store_registry_config:
            from hypha.litellm.vector_stores.vector_store_registry import VectorStoreRegistry
    ...
    def _load_alerting_settings(self, general_settings: dict):
        ...
        """
        from hypha.litellm.litellm_core_utils.litellm_logging import (
    ...
    def initialize_secret_manager(self, key_management_system: Optional[str]):
        ...
        if key_management_system is not None:
            if key_management_system == KeyManagementSystem.AZURE_KEY_VAULT.value:
                ...
                from hypha.litellm.secret_managers.aws_secret_manager_v2 import (
                    AWSSecretsManagerV2,
                )
                ...
                from hypha.litellm.secret_managers.google_secret_manager import (
                    GoogleSecretManager,
                )

                GoogleSecretManager()
            elif key_management_system == KeyManagementSystem.HASHICORP_VAULT.value:
                from hypha.litellm.secret_managers.hashicorp_secret_manager import (
    ...
    async def _add_router_settings_from_db_config(
        ...
        if llm_router is not None and prisma_client is not None:
            ...
            if (
                ...
                from hypha.litellm.utils import _update_dictionary
    ...
    async def _check_and_reload_model_cost_map(self, prisma_client: PrismaClient):
        ...
        try:
            ...
            if should_reload:
                # Perform the reload
                from hypha.litellm.litellm_core_utils.get_model_cost_map import (
                    get_model_cost_map,
                )
    ...
    async def _init_prompts_in_db(self, prisma_client: PrismaClient):
        from hypha.litellm.proxy.prompts.prompt_registry import IN_MEMORY_PROMPT_REGISTRY
    ...
    async def _init_guardrails_in_db(self, prisma_client: PrismaClient):
        from hypha.litellm.proxy.guardrails.guardrail_registry import (
    ...
    async def _init_mcp_servers_in_db(self):
        from hypha.litellm.proxy._experimental.mcp_server.utils import is_mcp_available
    ...
    async def delete_credentials(self, db_credentials: List[CredentialItem]):
        ...
        Delete any from hypha.litellm.credential_list that are not in the all-up list
        """
        ## CONFIG credentials ##
        config = await self.get_config(config_file_path=user_config_file_path)
...
async def initialize(  # noqa: PLR0915
    ...
    from hypha.litellm.proxy.common_utils.banner import show_banner
    ...
    if debug is True:  # this needs to be first, so users can see Router init debugg
        ...
        from hypha.litellm._logging import (
            verbose_logger,
...
class ProxyStartupEvent:
    ...
    async def _initialize_spend_tracking_background_jobs(
        ...
        from hypha.litellm.proxy.spend_tracking.cloudzero_endpoints import is_cloudzero_setup
    ...
    def _init_dd_tracer(cls):
        ...
        """
        from hypha.litellm.litellm_core_utils.dd_tracing import (
...
from hypha.litellm import _arealtime
...
from hypha.litellm.llms.base_llm.base_utils import BaseTokenCounter
...
def _get_provider_token_counter(
    ...
    from hypha.litellm.litellm_core_utils.get_llm_provider_logic import get_llm_provider
    ...
    try:
        ...
        from hypha.litellm.types.utils import LlmProviders
        from hypha.litellm.utils import ProviderConfigManager
        ...
        if full_model.startswith("anthropic/") or "anthropic" in full_model.lower():
            from hypha.litellm.llms.anthropic.common_utils import AnthropicModelInfo
...
async def token_counter(request: TokenCountRequest, call_endpoint: bool = False):
    ...
    Returns:
        TokenCountResponse
    """
    from hypha.litellm import token_counter
...
async def transform_request(request: TransformRequestBody):
    from hypha.litellm.utils import return_raw_request
...
async def model_group_info(
    ...
    from hypha.litellm.proxy.utils import get_available_models_for_user
...
async def fallback_login(request: Request):
    ...
    Example:
    """
    from hypha.litellm.proxy.proxy_server import ui_link
...
async def login(request: Request):  # noqa: PLR0915
    ...
    from hypha.litellm.types.proxy.ui_sso import ReturnedUITokenObject
...
async def new_invitation(
    ...
    """
    try:
        from hypha.litellm.proxy.management_helpers.user_invitation import (
...
async def reload_model_cost_map(
    ...
    try:
        ...
        from hypha.litellm.litellm_core_utils.get_model_cost_map import get_model_cost_map
...
async def get_routes():
    ...
    """
    from hypha.litellm.proxy.common_utils.get_routes import GetRoutes
