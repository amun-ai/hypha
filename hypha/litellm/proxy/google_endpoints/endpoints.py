...
from hypha.litellm.proxy.auth.user_api_key_auth import UserAPIKeyAuth, user_api_key_auth
from hypha.litellm.proxy.common_request_processing import ProxyBaseLLMRequestProcessing
from hypha.litellm.types.llms.vertex_ai import TokenCountDetailsResponse
...
async def google_count_tokens(request: Request, model_name: str):
    ...
    from hypha.litellm.proxy.proxy_server import token_counter as internal_token_counter
    from hypha.litellm.google_genai.adapters.transformation import GoogleGenAIAdapter

    data = await _read_request_body(request=request)
    contents = data.get("contents", [])
    #Create TokenCountRequest for the internal endpoint
    from hypha.litellm.proxy._types import TokenCountRequest
