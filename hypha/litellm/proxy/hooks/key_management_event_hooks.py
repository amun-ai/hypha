...
class KeyManagementEventHooks:
    ...
    async def async_key_generated_hook(
        ...
        """
        from hypha.litellm.proxy.management_helpers.audit_logs import (
            create_audit_log_for_update,
        )
        from hypha.litellm.proxy.proxy_server import litellm_proxy_admin_name
    ...
    async def _store_virtual_key_in_secret_manager(secret_name: str, secret_token: str):
        ...
        if litellm._key_management_settings is not None:
            if litellm._key_management_settings.store_virtual_keys is True:
                from hypha.litellm.secret_managers.base_secret_manager import (
                    BaseSecretManager,
                )
