...
from hypha.litellm.caching import DualCache
from hypha.litellm.constants import MAX_SPENDLOG_ROWS_TO_QUERY
...
from hypha.litellm.proxy.auth.auth_checks import ExperimentalUIJWTToken, get_user_object
from hypha.litellm.proxy.auth.auth_utils import _has_user_setup_sso
from hypha.litellm.proxy.auth.handle_jwt import JWTHandler
from hypha.litellm.proxy.auth.user_api_key_auth import user_api_key_auth
from hypha.litellm.proxy.common_utils.admin_ui_utils import (
    admin_ui_disabled,
    show_missing_vars_in_env,
)
from hypha.litellm.proxy.common_utils.html_forms.jwt_display_template import (
    jwt_display_template,
)
from hypha.litellm.proxy.common_utils.html_forms.ui_login import html_form
from hypha.litellm.proxy.management_endpoints.internal_user_endpoints import new_user
from hypha.litellm.proxy.management_endpoints.sso_helper_utils import (
    check_is_admin_only_access,
    has_admin_ui_access,
)
from hypha.litellm.proxy.management_endpoints.team_endpoints import new_team, team_member_add
from hypha.litellm.proxy.management_endpoints.types import CustomOpenID
...
from hypha.litellm.secret_managers.main import get_secret_bool, str_to_bool
...
async def google_login(
    ...
    Example:
    """
    from hypha.litellm.proxy.proxy_server import (
        premium_user,
        prisma_client,
...
async def auth_callback(request: Request, state: Optional[str] = None):  # noqa: PLR0915
    ...
    from hypha.litellm.constants import LITELLM_CLI_SESSION_TOKEN_PREFIX
    ...
    from hypha.litellm.proxy._types import LiteLLM_JWTAuth
    from hypha.litellm.proxy.auth.handle_jwt import JWTHandler
    from hypha.litellm.proxy.proxy_server import (
        general_settings,
        jwt_handler,
...
async def cli_sso_callback(request: Request, key: Optional[str] = None):
    ...
    try:
        ...
        from hypha.litellm.proxy.common_utils.html_forms.cli_sso_success import (
...
async def cli_poll_key(key_id: str):
    ...
    try:
        ...
        from hypha.litellm.proxy.utils import hash_token
...
async def get_ui_settings(request: Request):
    from hypha.litellm.proxy.proxy_server import general_settings, proxy_state
...
class SSOAuthenticationHandler:
    ...
    def get_redirect_url_for_sso(
        ...
        """
        from hypha.litellm.proxy.utils import get_custom_url
    ...
    async def get_redirect_response_from_openid(  # noqa: PLR0915
        ...
        from hypha.litellm.proxy.utils import get_prisma_client_or_throw
