...
from hypha.litellm.constants import LENGTH_OF_LITELLM_GENERATED_KEY, UI_SESSION_TOKEN_TEAM_ID
from hypha.litellm.litellm_core_utils.duration_parser import duration_in_seconds
...
from hypha.litellm.proxy.auth.auth_utils import abbreviate_api_key
from hypha.litellm.proxy.auth.user_api_key_auth import user_api_key_auth
from hypha.litellm.proxy.common_utils.timezone_utils import get_budget_reset_time
from hypha.litellm.proxy.hooks.key_management_event_hooks import KeyManagementEventHooks
from hypha.litellm.proxy.management_endpoints.common_utils import (
    _is_user_team_admin,
...
from hypha.litellm.proxy.management_helpers.object_permission_utils import (
    attach_object_permission_to_dict,
    handle_update_object_permission_common,
)
from hypha.litellm.proxy.management_helpers.team_member_permission_checks import (
    TeamMemberPermissionChecks,
)
from hypha.litellm.proxy.management_helpers.utils import management_endpoint_wrapper
from hypha.litellm.proxy.spend_tracking.spend_tracking_utils import _is_master_key
...
from hypha.litellm.types.router import Deployment
    ...
    try:
        for k, v in data_json.items():
            ...
            if k in LiteLLM_ManagementEndpoint_MetadataFields_Premium:
                from hypha.litellm.proxy.utils import _premium_user_check
...
async def delete_key_fn(
    ...
    """
    try:
        from hypha.litellm.proxy.proxy_server import prisma_client, user_api_key_cache
...
async def generate_key_helper_fn(  # noqa: PLR0915
    ...
    from hypha.litellm.proxy.proxy_server import premium_user, prisma_client
...
async def test_key_logging(
    ...
    from hypha.litellm.proxy.proxy_server import general_settings, proxy_config
...
def validate_model_max_budget(model_max_budget: Optional[Dict]) -> None:
    ...
    try:
        ...
        if model_max_budget is not None:
            from hypha.litellm.proxy.proxy_server import CommonProxyErrors, premium_user
