...
import hypha.litellm._service_logger  # for storing API inputs, outputs, and metadata
import hypha.litellm.litellm_core_utils
import hypha.litellm.litellm_core_utils.audio_utils.utils
import hypha.litellm.litellm_core_utils.json_validation_rule
import hypha.litellm.llms
import hypha.litellm.llms.gemini
from hypha.litellm.caching._internal_lru_cache import lru_cache_wrapper
from hypha.litellm.caching.caching import DualCache
from hypha.litellm.caching.caching_handler import CachingHandlerResponse, LLMCachingHandler
...
from hypha.litellm.integrations.custom_guardrail import CustomGuardrail
from hypha.litellm.integrations.custom_logger import CustomLogger
from hypha.litellm.integrations.vector_store_integrations.base_vector_store import (
    BaseVectorStore,
)
...
from hypha.litellm.litellm_core_utils.default_encoding import encoding
from hypha.litellm.litellm_core_utils.exception_mapping_utils import (
    _get_response_headers,
    exception_type,
    get_error_message,
)
from hypha.litellm.litellm_core_utils.get_litellm_params import (
    _get_base_model_from_litellm_call_metadata,
    get_litellm_params,
)
from hypha.litellm.litellm_core_utils.get_llm_provider_logic import (
    _is_non_openai_azure_model,
    get_llm_provider,
)
from hypha.litellm.litellm_core_utils.get_supported_openai_params import (
    get_supported_openai_params,
)
from hypha.litellm.litellm_core_utils.llm_request_utils import _ensure_extra_body_is_safe
from hypha.litellm.litellm_core_utils.llm_response_utils.convert_dict_to_response import (
...
from hypha.litellm.litellm_core_utils.llm_response_utils.get_api_base import get_api_base
from hypha.litellm.litellm_core_utils.llm_response_utils.get_formatted_prompt import (
    get_formatted_prompt,
)
from hypha.litellm.litellm_core_utils.llm_response_utils.get_headers import (
    get_response_headers,
)
from hypha.litellm.litellm_core_utils.llm_response_utils.response_metadata import (
    ResponseMetadata,
)
from hypha.litellm.litellm_core_utils.redact_messages import (
    LiteLLMLoggingObject,
    redact_message_input_output_from_logging,
)
from hypha.litellm.litellm_core_utils.rules import Rules
from hypha.litellm.litellm_core_utils.streaming_handler import CustomStreamWrapper
from hypha.litellm.litellm_core_utils.token_counter import get_modified_max_tokens
from hypha.litellm.llms.base_llm.google_genai.transformation import (
    BaseGoogleGenAIGenerateContentConfig,
)
from hypha.litellm.llms.bedrock.common_utils import BedrockModelInfo
from hypha.litellm.llms.custom_httpx.http_handler import AsyncHTTPHandler, HTTPHandler
from hypha.litellm.router_utils.get_retry_from_policy import (
    get_num_retries_from_retry_policy,
    reset_retry_policy,
)
from hypha.litellm.secret_managers.main import get_secret
from hypha.litellm.types.llms.anthropic import (
    ANTHROPIC_API_ONLY_HEADERS,
    AnthropicThinkingParam,
)
from hypha.litellm.types.llms.openai import (
    AllMessageValues,
    AllPromptValues,
...
from hypha.litellm.types.rerank import RerankResponse
from hypha.litellm.types.utils import FileTypes  # type: ignore
...
from hypha.litellm.litellm_core_utils.thread_pool_executor import executor
from hypha.litellm.litellm_core_utils.token_counter import token_counter as token_counter_new
from hypha.litellm.llms.base_llm.anthropic_messages.transformation import (
    BaseAnthropicMessagesConfig,
)
from hypha.litellm.llms.base_llm.audio_transcription.transformation import (
    BaseAudioTranscriptionConfig,
)
from hypha.litellm.llms.base_llm.base_utils import (
    BaseLLMModelInfo,
    type_to_response_format_param,
)
from hypha.litellm.llms.base_llm.batches.transformation import BaseBatchesConfig
from hypha.litellm.llms.base_llm.chat.transformation import BaseConfig
from hypha.litellm.llms.base_llm.completion.transformation import BaseTextCompletionConfig
from hypha.litellm.llms.base_llm.embedding.transformation import BaseEmbeddingConfig
from hypha.litellm.llms.base_llm.files.transformation import BaseFilesConfig
from hypha.litellm.llms.base_llm.image_edit.transformation import BaseImageEditConfig
from hypha.litellm.llms.base_llm.image_generation.transformation import (
    BaseImageGenerationConfig,
)
from hypha.litellm.llms.base_llm.image_variations.transformation import (
    BaseImageVariationConfig,
)
from hypha.litellm.llms.base_llm.passthrough.transformation import BasePassthroughConfig
from hypha.litellm.llms.base_llm.realtime.transformation import BaseRealtimeConfig
from hypha.litellm.llms.base_llm.rerank.transformation import BaseRerankConfig
from hypha.litellm.llms.base_llm.responses.transformation import BaseResponsesAPIConfig
from hypha.litellm.llms.base_llm.vector_store.transformation import BaseVectorStoreConfig
...
def function_setup(  # noqa: PLR0915
    ...
    ### NOTICES ###
    from hypha.litellm import Logging as LiteLLMLogging
    from hypha.litellm.litellm_core_utils.litellm_logging import set_callbacks
    ...
    try:
        ...
        if add_breadcrumb:
            try:
                from hypha.litellm.litellm_core_utils.core_helpers import safe_deep_copy
...
async def _client_async_logging_helper(
    ...
    if (
        ...
        ################################################
        from hypha.litellm.litellm_core_utils.logging_worker import GLOBAL_LOGGING_WORKER
        ...
        try:
            ...
            if call_type == CallTypes.completion.value:
                ...
                _is_litellm_router_call = "model_group" in kwargs.get(
                    "metadata", {}
                )  # check if call from hypha.litellm.router/proxy
                if (
...
def get_optional_params_transcription(
    ...
    **kwargs,
    ...
    from hypha.litellm.constants import OPENAI_TRANSCRIPTION_PARAMS
...
from hypha.litellm.caching.in_memory_cache import InMemoryCache
...
class ProviderConfigManager:
    ...
    def get_provider_chat_config(  # noqa: PLR0915
        ...
        if (
            ...
            if "gemini" in model:
                ...
                from hypha.litellm.llms.vertex_ai.vertex_ai_partner_models.gpt_oss.transformation import (
            ...
            if bedrock_route == "converse" or bedrock_route == "converse_like":
                ...
                from hypha.litellm.llms.bedrock.chat.invoke_agent.transformation import (
    ...
    def get_provider_embedding_config(
        ...
        if (
            ...
            from hypha.litellm.llms.cohere.embed.transformation import CohereEmbeddingConfig

            return CohereEmbeddingConfig()
        elif litellm.LlmProviders.JINA_AI == provider:
            from hypha.litellm.llms.jina_ai.embedding.transformation import (
                JinaAIEmbeddingConfig,
            )

            return JinaAIEmbeddingConfig()
        elif litellm.LlmProviders.VOLCENGINE == provider:
            from hypha.litellm.llms.volcengine.embedding.transformation import (
    ...
    def get_provider_anthropic_messages_config(
        ...
        # This mapping ensures that the correct configuration is returned for BEDROCK.
            ...
            if "claude" in model:
                from hypha.litellm.llms.vertex_ai.vertex_ai_partner_models.anthropic.experimental_pass_through.transformation import (
    ...
    def get_provider_audio_transcription_config(
        ...
        if litellm.LlmProviders.FIREWORKS_AI == provider:
            ...
            from hypha.litellm.llms.elevenlabs.audio_transcription.transformation import (
    ...
    def get_provider_model_info(
        ...
        if LlmProviders.FIREWORKS_AI == provider:
            ...
            from hypha.litellm.llms.vertex_ai.common_utils import VertexAIModelInfo
            ...
            from hypha.litellm.llms.ollama.common_utils import OllamaModelInfo

            return OllamaModelInfo()
        elif LlmProviders.VLLM == provider or LlmProviders.HOSTED_VLLM == provider:
            from hypha.litellm.llms.vllm.common_utils import (
    ...
    def get_provider_passthrough_config(
        ...
        if LlmProviders.BEDROCK == provider:
            from hypha.litellm.llms.bedrock.passthrough.transformation import (
                BedrockPassthroughConfig,
            )

            return BedrockPassthroughConfig()
        elif LlmProviders.VLLM == provider:
            from hypha.litellm.llms.vllm.passthrough.transformation import (
    ...
    def get_provider_files_config(
        ...
        if LlmProviders.GEMINI == provider:
            from hypha.litellm.llms.gemini.files.transformation import (
                GoogleAIStudioFilesHandler,  # experimental approach, to reduce bloat on __init__.py
            )

            return GoogleAIStudioFilesHandler()
        elif LlmProviders.VERTEX_AI == provider:
            from hypha.litellm.llms.vertex_ai.files.transformation import VertexAIFilesConfig

            return VertexAIFilesConfig()
        elif LlmProviders.BEDROCK == provider:
            from hypha.litellm.llms.bedrock.files.transformation import BedrockFilesConfig
    ...
    def get_provider_batches_config(
        ...
        if LlmProviders.BEDROCK == provider:
            from hypha.litellm.llms.bedrock.batches.transformation import BedrockBatchesConfig
    ...
    def get_provider_vector_store_config(
        ...
        from hypha.litellm.integrations.vector_store_integrations.bedrock_vector_store import (
            BedrockVectorStore,
        )
    ...
    def get_provider_vector_stores_config(
        ...
        """
        if litellm.LlmProviders.OPENAI == provider:
            from hypha.litellm.llms.openai.vector_stores.transformation import (
                OpenAIVectorStoreConfig,
            )

            return OpenAIVectorStoreConfig()
        elif litellm.LlmProviders.AZURE == provider:
            from hypha.litellm.llms.azure.vector_stores.transformation import (
                AzureOpenAIVectorStoreConfig,
            )

            return AzureOpenAIVectorStoreConfig()
        elif litellm.LlmProviders.VERTEX_AI == provider:
            from hypha.litellm.llms.vertex_ai.vector_stores.transformation import (
                VertexVectorStoreConfig,
            )

            return VertexVectorStoreConfig()
        elif litellm.LlmProviders.BEDROCK == provider:
            from hypha.litellm.llms.bedrock.vector_stores.transformation import (
                BedrockVectorStoreConfig,
            )

            return BedrockVectorStoreConfig()
        elif litellm.LlmProviders.PG_VECTOR == provider:
            from hypha.litellm.llms.pg_vector.vector_stores.transformation import (
                PGVectorStoreConfig,
            )
    ...
    def get_provider_image_generation_config(
        ...
        if LlmProviders.OPENAI == provider:
            ...
            from hypha.litellm.llms.xinference.image_generation import (
    ...
    def get_provider_realtime_config(
        ...
        if LlmProviders.GEMINI == provider:
            from hypha.litellm.llms.gemini.realtime.transformation import GeminiRealtimeConfig
    ...
    def get_provider_image_edit_config(
        ...
        if LlmProviders.OPENAI == provider:
            from hypha.litellm.llms.openai.image_edit.transformation import (
                OpenAIImageEditConfig,
            )

            return OpenAIImageEditConfig()
        elif LlmProviders.AZURE == provider:
            from hypha.litellm.llms.azure.image_edit.transformation import (
                AzureImageEditConfig,
            )

            return AzureImageEditConfig()
        elif LlmProviders.RECRAFT == provider:
            from hypha.litellm.llms.recraft.image_edit.transformation import (
                RecraftImageEditConfig,
            )

            return RecraftImageEditConfig()
        elif LlmProviders.LITELLM_PROXY == provider:
            from hypha.litellm.llms.litellm_proxy.image_edit.transformation import (
    ...
    def get_provider_google_genai_generate_content_config(
        ...
        if litellm.LlmProviders.GEMINI == provider:
            from hypha.litellm.llms.gemini.google_genai.transformation import (
                GoogleGenAIConfig,
            )

            return GoogleGenAIConfig()
        elif litellm.LlmProviders.VERTEX_AI == provider:
            from hypha.litellm.llms.vertex_ai.google_genai.transformation import (
                VertexAIGoogleGenAIConfig,
            )
            from hypha.litellm.llms.vertex_ai.vertex_ai_partner_models.main import (
...
def return_raw_request(endpoint: CallTypes, kwargs: dict) -> RawRequestTypedDict:
    ...
    from hypha.litellm.litellm_core_utils.litellm_logging import Logging
