...
from hypha.core.auth import generate_presigned_token, create_scope, generate_anonymous_user
...
def create_cli_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="hypha-cli", 
        description="Hypha server and CLI tools"
    )
    ...
    token_parser = subparsers.add_parser("generate-token", help="Generate authentication token")
    ...
    token_parser.add_argument(
        "--client-id", 
        type=str, 
        help="Optional client ID to restrict the token"
    )
    token_parser.add_argument(
        "--user-id", 
        type=str, 
        help="User ID (default: generated anonymous user)"
    )
    token_parser.add_argument(
        "--role", 
        type=str, 
