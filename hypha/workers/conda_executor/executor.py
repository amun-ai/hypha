...
class CondaEnvExecutor:
    ...
    @classmethod
    def create_temp_env(cls, dependencies: List[Union[str, Dict]], channels: Optional[List[str]] = None, name: str = None, **kwargs) -> 'CondaEnvExecutor':
    ...
    async def _create_conda(self, progress_callback=None) -> float:
        ...
        if str(self.env_source).endswith(('.tar.gz', '.tgz')):
            if progress_callback:
                progress_callback({
                    "type": "info",
                    "message": "Extracting conda-pack file..."
                })
            try:
        ...
        if progress_callback:
            progress_callback({
                "type": "info",
                "message": "Preparing environment specification..."
            })
        ...
        try:
            ...
            if return_code == 0:
                ...
                log.info(f"Successfully created environment using {self.package_manager}: {self.env_path}")
                return time.time() - start_time
            else:
            ...
            error_msg = f"Failed to create environment using {self.package_manager}: {str(e)}"
            if progress_callback:
                progress_callback({
                    "type": "error",
                    "message": error_msg
    ...
    def _build_create_command(self, env_file):
        ...
        base_cmd = f"{self.package_manager} env create -p {self.env_path} -f {env_file} -y"
        ...
        if os.name == 'nt':  # Windows
            ...
            if self.package_manager == 'mamba':
                ...
                if shell_executable:
                    try:
                        info_base_cmd = get_package_manager_info_base(self.package_manager)
                        # Use proper path separator for the platform
                        conda_sh_path = "etc/profile.d/conda.sh"
                        sourced_cmd = f"source $({info_base_cmd})/{conda_sh_path} && {base_cmd}"
    ...
    async def _run_subprocess_with_streaming(self, command, progress_callback=None, executable=None):
        """Run subprocess with real-time output streaming.
        ...
        try:
            log.info(f"Starting subprocess: {' '.join(cmd) if isinstance(cmd, list) else cmd}")
            ...
            try:
                ...
                if return_code == 0:
                    log.info(f"Subprocess completed successfully with return code {return_code}")
                else:
            ...
            if progress_callback:
                try:
                    progress_callback({
                        "type": "error",
                        "message": f"Failed to start subprocess: {e}"
                    })
    ...
    def _classify_message_type(self, message):
        ...
        # Success indicators
        success_keywords = ['done', 'complete', 'success', 'finished', 'transaction finished', 'linking']
    ...
    async def _extract_env(self, progress_callback=None) -> float:
        ...
        if os.path.exists(python_path):
            if progress_callback:
                progress_callback({
                    "type": "info",
                    "message": "Validating existing environment..."
                })
            try:
